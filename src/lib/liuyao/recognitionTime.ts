...
const toLocalDateIso = (y: number, m: number, d: number, h: number, min: number) => {
  ...
  // Guard against Date auto-correction (e.g. 2026-02-30 -> 2026-03-02).
  if (
    dt.getFullYear() !== y ||
    dt.getMonth() !== m - 1 ||
    dt.getDate() !== d ||
  ...
  return { dateIso: dt.toISOString(), timeHHmm: `${pad2(h)}:${pad2(min)}` };
};
...
const parseYmdhmFromText = (raw: string) => {
  ...
  // YYYY年MM月DD日 HH:mm / H点mm / H时mm / ... (minute required)
  const zh = s.match(
...
export const parseLiuyaoDateTimeFromIsoLike = (input: string) => {
  ...
  // Prefer native ISO parsing when possible.
  const direct = new Date(maybe);
  if (!Number.isNaN(direct.getTime())) {
    return { dateIso: direct.toISOString(), timeHHmm: `${pad2(direct.getHours())}:${pad2(direct.getMinutes())}` };
  }
...
export const parseLiuyaoDateTimeFromSolarLike = (solar: unknown) => {
  if (!solar || typeof solar !== "object") return null;
  const obj = solar as Record<string, unknown>;
  const y = coerceInt(obj.y);
  const m = coerceInt(obj.m);
  const d = coerceInt(obj.d);
  const h = coerceInt(obj.h);
  const min = coerceInt(obj.min);
