  ...
  const combinedTimeline = useMemo(() => {
    ...
    const customerEvents = events
      ...
      .map(e => ({
        ...
        type: 'event' as const,
        dateDisplay: e.time,
        timestamp: 0, // 历史大事通常只有模糊年份，默认排在记录之前或特殊处理
        title: e.description,
        content: '',
        tags: e.tags
      }));
    ...
    const customerRecords = records
      ...
      .map(r => ({
        ...
        dateDisplay: new Date(r.createdAt).toLocaleDateString('zh-CN', { year: 'numeric', month: 'long', day: 'numeric' }),
    ...
    // 排序逻辑：记录按时间戳降序，大事记按描述中的年份尝试排序
    return [...customerEvents, ...customerRecords].sort((a, b) => {
      if (a.type === 'consultation' && b.type === 'consultation') return b.timestamp - a.timestamp;
      if (a.type === 'event' && b.type === 'event') return b.dateDisplay.localeCompare(a.dateDisplay);
      return a.type === 'event' ? -1 : 1; // 大事记通常放在最上方或最下方，这里视大事记为背景
    });
  }, [id, events, records]);
    ...
    <div className="space-y-12 pb-20">
      <header className="flex justify-between items-end border-b border-[#B37D56]/10 pb-8">
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-4">
            <h2 className="text-4xl font-bold text-[#2F2F2F] chinese-font tracking-widest">{customer.name} · 历程纪</h2>
        ...
        <div className="hidden md:block text-right">
          <div className="p-4 bg-white border border-[#B37D56]/10 inline-block">
             ...
             <p className="text-lg font-bold text-[#2F2F2F] chinese-font">已随行 {Math.floor((Date.now() - customer.createdAt) / (1000 * 60 * 60 * 24))} 日</p>
          </div>
        </div>
      </header>
      ...
      <div className="relative max-w-4xl mx-auto pl-8 md:pl-0">
        ...
        <div className="space-y-16">
          {combinedTimeline.length > 0 ? combinedTimeline.map((item, index) => (
            <div key={item.id} className={`relative flex flex-col md:flex-row items-start md:items-center ${index % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
              ...
              <div className={`w-full md:w-[45%] ${index % 2 === 0 ? 'md:pr-12 text-left md:text-right' : 'md:pl-12 text-left'}`}>
                <div className="bg-white p-6 border border-[#B37D56]/10 hover:border-[#A62121]/30 transition-all shadow-sm group">
                  <div className={`flex items-center gap-2 mb-2 ${index % 2 === 0 ? 'md:justify-end' : 'justify-start'}`}>
                    <span className="text-[10px] font-bold text-[#A62121] tracking-[0.2em] uppercase">{item.dateDisplay}</span>
              ...
              {/* Date for the other side on Desktop */}
