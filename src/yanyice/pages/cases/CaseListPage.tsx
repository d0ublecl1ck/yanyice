    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row gap-6 items-center">
        <div className="relative flex-1 group">
          <Search size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-[#B37D56]/40 group-focus-within:text-[#A62121] transition-colors" />
          <input 
            type="text" 
            value={search}
        ...
        <div className="flex gap-2">
          {(['all', 'liuyao', 'bazi'] as const).map(f => (
            <button 
              key={f}
              onClick={() => setFilter(f)}
              className={`px-6 py-3 text-xs font-bold tracking-widest border transition-all ${filter === f ? 'bg-[#2F2F2F] text-white border-[#2F2F2F]' : 'bg-white text-[#2F2F2F]/40 border-[#B37D56]/10 hover:border-[#A62121]'}`}
            >
      ...
      <div className="bg-white border border-[#B37D56]/10 rounded-none shadow-sm overflow-hidden">
        {filteredRecords.length > 0 ? (
          <div className="divide-y divide-[#B37D56]/5">
            {filteredRecords.sort((a,b) => b.createdAt - a.createdAt).map(record => {
              ...
              return (
                ...
                >
                  <div className="w-12 h-12 bg-[#B37D56]/5 flex items-center justify-center shrink-0">
                    <FileText size={20} className="text-[#B37D56]/30 group-hover:text-[#A62121] transition-colors" />
                  </div>
                  <div className="ml-6 flex-1 grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
                    <div className="md:col-span-2">
                      <h4 className="font-bold text-[#2F2F2F] chinese-font group-hover:text-[#A62121] transition-colors">{record.subject}</h4>
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-[10px] font-bold text-[#B37D56] uppercase tracking-widest">{customer?.name || '未知客户'}</span>
                        <span className="text-[10px] text-[#2F2F2F]/20">|</span>
                        <span className="text-[10px] text-[#2F2F2F]/30">{record.module === 'liuyao' ? '文王六爻' : '子平八字'}</span>
                      </div>
                    </div>
                    ...
                    <div className="text-right">
                      <span className={`text-[9px] px-2 py-0.5 border font-bold ${
                        record.verifiedStatus === 'accurate' ? 'border-[#8DA399] text-[#8DA399]' : 'border-[#B37D56]/20 text-[#2F2F2F]/30'
                      }`}>
                        {record.verifiedStatus === 'accurate' ? '已反馈: 准确' : '未反馈'}
                      </span>
                    </div>
                  </div>
                  <ChevronRight size={16} className="ml-6 text-[#2F2F2F]/10 group-hover:text-[#A62121] transition-all transform group-hover:translate-x-1" />
                </Link>
              );
            })}
          </div>
